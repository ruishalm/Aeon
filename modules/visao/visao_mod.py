...
class VisaoModule(AeonModule):
    """
    Módulo unificado de Visão.
    - Analisa screenshots e imagens da webcam.
    - Controla o sistema via gestos manuais.
    """
    def __init__(self, core_context):
        ...
        self.triggers = [
            ...
            # Controle de Gestos
            "ativar visão", "ligar câmera", "parar visão", "desligar câmera",
            "modo gestos", "testar câmera", "ver pela câmera", "acessar câmera"
        ]
        self.dependencies = ["brain", "context", "gui", "io_handler"]
    ...
    def check_dependencies(self):
        ...
        if not VISAO_AVAILABLE:
            ...
            # Não impede o carregamento, apenas limita a funcionalidade.
        if not GEAR_AVAILABLE:
            print("[VISAO] Aviso: 'opencv-python' ou 'mediapipe' não instalados. Módulo de gestos desativado.")
            # Não impede o carregamento.
        
        # O módulo pode funcionar parcialmente, então não retornamos False aqui.
        # A checagem real acontece no 'process'.
    ...
    def process(self, command: str) -> str:
        ...
        # 1. Comandos de Gestos
        gesture_commands_start = ["modo gestos", "ativar gestos", "ligar gestos", "testar câmera", "ativar visão", "ligar câmera"]
        gesture_commands_stop = ["parar visão", "parar gestos", "desligar câmera"]
        
        if any(c in cmd for c in gesture_commands_start):
            if not GEAR_AVAILABLE:
    ...
    # ===============================================
    # MÉTODOS DE ANÁLISE DE IMAGEM (FRAME ÚNICO)
    # ===============================================
    def _capture_webcam_frame(self) -> bytes:
        """Captura um único frame da webcam e retorna em bytes JPEG."""
        cap = cv2.VideoCapture(0)
        if not cap.isOpened():
            raise Exception("Não consegui acessar a câmera.")
        
        for _ in range(5): cap.read() # Aquecimento
        ret, frame = cap.read()
        cap.release()
        ...
        if not ret:
            raise Exception("Erro ao codificar imagem.")
        return buffer.tobytes()

    # ===============================================
    # MÉTODOS DE GESTOS (LOOP CONTÍNUO)
    # ===============================================
    def _initialize_detector(self):
        ...
        if not os.path.exists(self.model_path) or os.path.getsize(self.model_path) < 1000:
            ...
            if not os.path.exists(self.model_path):
                print(f"[VISAO][ERRO] Falha ao obter modelo de gestos.")
                self.detector = None
                return
        ...
        try:
            ...
            print("[VISAO] Motor de Gestos (MediaPipe) inicializado.")
        except Exception as e:
            print(f"[VISAO][ERRO] Falha ao inicializar MediaPipe: {e}")
            self.detector = None
    ...
    def _gesture_vision_loop(self):
        ...
        if not self.cap.isOpened():
            print("[VISAO][ERRO] Não foi possível acessar a câmera.")
            self.running = False
    ...
    def _executar_acao(self, gesto):
        ...
        print(f"[VISAO] Gesto '{gesto}' detectado, executando ação.")
